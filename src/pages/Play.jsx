...
import { BOX_COUNT, REWARD_MUTIPLIER, BET_RANGE, EARNING_RATE } from '../config';
...
const Play = () => {
  ...
  const handleSelectABox = async (boxIndex) => {
    ...
    if (gameResult === false) {
      ...
      if (losePlayIndex >= 9007199254740990) dispatch(setLosePlayIndex(Number(0)));
      ...
      if (losePlayIndex >= 9007199254740990) dispatch(setWinPlayIndex(Number(0)));
      ...
      recordPlayHistory({
        ...
        winOrLose: true,
        boxes: numberOfBoxs,
        resultAmount: depositAmount >= 0 ? depositAmount * EARNING_RATE[numberOfBoxs] * 0.965 : 0,
      });
      NotificationManager.info(
        `You earned ${
          depositAmount >= 0
            ? Number(depositAmount * EARNING_RATE[numberOfBoxs] * 0.965).toFixed(3)
            : '0'
      ...
      setWorking(false);
      NotificationManager.error(`Transaction is rejected by user`, 'Warn!', 3000, () => {
        resetGame();
      });
      setTimeout(() => {
        resetGame();
      ...
      <div className="flex flex-col justify-center items-center text-center ">
        ...
        <div className="w-full md:w-[450px] pb-10 flex gap-1 md:gap-2 justify-center cursor-pointer">
          ...
          <button
            ...
            onClick={() => onChangeNumberOfBoxs(8, 4)}>
            8 Boxes
          </button>
          <button
            className={`optionbutton ${
